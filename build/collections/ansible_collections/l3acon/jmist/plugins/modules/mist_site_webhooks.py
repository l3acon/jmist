...
"""The module file for mist_site_webhooks"""
...
DOCUMENTATION = """
---
module: mist_site_webhooks
short_description: Manage webhooks in Juniper Mist
description:
  - Manage Sites Webhooks resources in Juniper Mist
...
- name: Create a webhooks resource
  l3acon.jmist.mist_site_webhooks:
...
def main():
    ...
    if state == 'gathered':
        if module.check_mode:
            ...
            response = client.get('/api/v1/sites/{site_id}/webhooks'.format(site_id=site_id))
        ...
        if resource_id:
            if not module.check_mode:
                response = client.put(
                    '/api/v1/sites/{site_id}/webhooks/{webhook_id}'.format(site_id=site_id, webhook_id=resource_id),
                    data=payload,
                )
            ...
            if not module.check_mode:
                response = client.post(
                    '/api/v1/sites/{site_id}/webhooks'.format(site_id=site_id),
                    data=payload,
                )
                result['response'] = response
            result['changed'] = True
