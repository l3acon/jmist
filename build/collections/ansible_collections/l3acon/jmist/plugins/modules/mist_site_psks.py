...
"""The module file for mist_site_psks"""
...
DOCUMENTATION = """
---
module: mist_site_psks
short_description: Manage psks in Juniper Mist
description:
  - Manage Sites Psks resources in Juniper Mist
...
def main():
    ...
    if state == 'gathered':
        if module.check_mode:
            ...
            response = client.get('/api/v1/sites/{site_id}/psks'.format(site_id=site_id))
        ...
        if resource_id:
            if not module.check_mode:
                response = client.put(
                    '/api/v1/sites/{site_id}/psks/{psk_id}'.format(site_id=site_id, psk_id=resource_id),
                    data=payload,
                )
            ...
            if not module.check_mode:
                response = client.post(
                    '/api/v1/sites/{site_id}/psks'.format(site_id=site_id),
                    data=payload,
                )
                result['response'] = response
            result['changed'] = True
