...
"""The module file for mist_org_vpns"""
...
DOCUMENTATION = """
---
module: mist_org_vpns
short_description: Manage vpns in Juniper Mist
description:
  - Manage Orgs VPNs resources in Juniper Mist
  - This module supports state-based management of vpns resources.
version_added: "1.0.0"
author:
...
options:
    ...
    vpn_id:
      description:
        - "Unique identifier of the vpns resource. Required for state=absent."
      type: str
    state:
      description:
...
EXAMPLES = """
---
- name: Create a vpns resource
  l3acon.jmist.mist_org_vpns:
    org_id: "your-org-id"
    state: present
    name: "example-vpns"

- name: Delete a vpns resource
  l3acon.jmist.mist_org_vpns:
    org_id: "your-org-id"
    vpn_id: "resource-uuid"
    state: absent

- name: Gather vpns resources
  l3acon.jmist.mist_org_vpns:
    org_id: "your-org-id"
    state: gathered
"""
...
def main():
    argument_spec = dict(
        ...
        name=dict(type='str'),
        path_selection=dict(type='dict'),
        paths=dict(type='dict'),
        type=dict(type='str', choices=['hub_spoke', 'mesh'], default='hub_spoke'),
    )
    ...
    resource_id = module.params.get('vpn_id')
    ...
    if state == 'gathered':
        if module.check_mode:
            ...
            response = client.get('/api/v1/orgs/{org_id}/vpns'.format(org_id=org_id))
        ...
        if resource_id:
            if not module.check_mode:
                response = client.put(
                    '/api/v1/orgs/{org_id}/vpns/{vpn_id}'.format(org_id=org_id, vpn_id=resource_id),
                    data=payload,
                )
            ...
            if not module.check_mode:
                response = client.post(
                    '/api/v1/orgs/{org_id}/vpns'.format(org_id=org_id),
                    data=payload,
                )
                result['response'] = response
            result['changed'] = True
        ...
        if not resource_id:
            module.fail_json(msg="vpn_id is required when state=absent")
        if not module.check_mode:
            client.delete(
                '/api/v1/orgs/{org_id}/vpns/{vpn_id}'.format(org_id=org_id, vpn_id=resource_id),
            )
